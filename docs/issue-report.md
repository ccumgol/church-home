...
### 🔍 근본 원인
- CSS 내 메뉴바 불투명화 규칙을 적용하는 과정에서 `nav.navbar ul` 및 `.navbar-menu` 서브메뉴 전체에 `visibility: visible !important; opacity: 1 !important;` 속성이 잘못 주입되어, 숨겨져 있어야 할 모든 하위 <ul> 드롭다운 레이어가 일괄 강제 열림 상태가 되었습니다.

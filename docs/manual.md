...
4. [주일설교 4단 동적 태그 레이아웃 & 영상 축적 가이드](#주일설교-4단-동적-태그-레이아웃--영상-축적-가이드)
...
## 4. 주일설교 4단 동적 태그 레이아웃 & 영상 축적 가이드

`content/sermons/` 디렉터리에 새로운 `.md` 파일을 추가하는 것만으로 4단 분할(좌측 3단 설교 카드 + 우측 4번째 단 동적 태그 사이드바) 레이아웃에 자동 축적됩니다.

### 4.1 주일설교 동적 태그 자동 추출 원리
- **`book:` (성경권별):** 설교 글의 `book` 속성에 적힌 성경권(예: `요한복음`, `마태복음`)을 자동 추출하여 사이드바에 실시간 태그 버튼 생성.
- **`speaker:` (설교자별):** `speaker` 속성에 적힌 설교자(예: `담임목사`)를 자동 추출.
- **`tags:` (절기별):** `tags` 배열에 적힌 절기(예: `["주일대예배", "부활절"]`)를 자동 추출.
- **자동 정리:** 현재 게시물들에 **실제 존재하는 태그만 중복 없이 자동 수집 및 정렬**되어 우측 사이드바에 흰색 버튼 형태로 실시간 노출됩니다.
